...
wordswap: #s2 counter
    ...
    #multiply counter by 4 and subtract 4 to find destination address
    slli t2, s4, 2 
    addi t2, t2, -4
    add t2, t2, s1 #address of destination register

    #multiply counter by 4 for current register address 
    slli t3, s5, 2
    ...
    sw t3, 0(t2) #store current word @ destination address
    sw t2, 0(t3) #store destination word @ current address

    addi s4, s4, -1 # decrement counter
    addi s1, s1, 4 # increment word register address
    j wordswap
